...
// Installs a guest-call trap on [engine]: whenever guest code attempts to
...
// that address, we return false and let the real fault surface (through
// stepEngine()'s normal fault path in cpu_bridge.cpp), so genuine bugs
// still surface as crashes rather than being silently swallowed. See
// mre/VmDispatcher.kt for the full rationale and the registered
// (placeholder) address table.
//
// IMPORTANT - THREAD AFFINITY: [env] is cached for reuse inside the hook
// callback, which fires synchronously during vxp_run()/vxp_step() on
// whatever thread called it. This function MUST be called from the same
// thread that will later call Executor.run()/step() (i.e. call this from
