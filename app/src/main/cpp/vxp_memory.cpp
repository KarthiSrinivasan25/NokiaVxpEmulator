...
namespace {
inline uint64_t pageIndexOf(uint64_t address) {
    return address / VxpMemory::VXP_PAGE_SIZE;
}

inline uint64_t alignDown(uint64_t addr) {
    return addr & ~(VxpMemory::VXP_PAGE_SIZE - 1);
}
} // namespace
...
bool VxpMemory::mapRegion(uint64_t base, uint64_t size, uint32_t perms,
    ...
    uint64_t pageCount = alignedSize / VxpMemory::VXP_PAGE_SIZE;
    ...
    if (initialData != nullptr && initialLen > 0) {
        // Bypasses permission checks - matches uc_mem_write's
        // "backdoor" semantics.
...
bool VxpMemory::apiRead(uint64_t address, uint8_t* out, size_t len) const {
    for (size_t i = 0; i < len; i++) {
        ...
        out[i] = page
            ? page->data[(address + i) % VxpMemory::VXP_PAGE_SIZE]
            : 0;
...
bool VxpMemory::apiWrite(uint64_t address, const uint8_t* data, size_t len) {
    for (size_t i = 0; i < len; i++) {
        ...
        // apiWrite is used both for initial segment loading (into pages
        // we just created, with whatever perms mapRegion set) and generic
        ...
        if (page->perms == 0) {
            page->perms = VXP_PROT_READ | VXP_PROT_WRITE;
        }

        page->data[addr % VxpMemory::VXP_PAGE_SIZE] = data[i];
    }

    return true;
}

bool VxpMemory::accessChecked(uint32_t address, uint8_t* out,
                              const uint8_t* in, size_t len,
                              uint32_t requiredPerm,
                              ArmFault unmappedFault,
                              ArmFault protFault,
                              ArmFault* fault) {
    for (size_t i = 0; i < len; i++) {
        ...
        if ((page->perms & requiredPerm) == 0) {
            *fault = protFault;
            return false;
        }

        if (out != nullptr) {
            out[i] = page->data[addr % VxpMemory::VXP_PAGE_SIZE];
        }

        if (in != nullptr) {
            page->data[addr % VxpMemory::VXP_PAGE_SIZE] = in[i];
        }
    }
...
bool VxpMemory::read(uint32_t address, uint8_t* out, size_t len,
                     ArmFault* fault) {
    return accessChecked(
        address, out, nullptr, len,
        VXP_PROT_READ,
        ArmFault::ReadUnmapped,
        ArmFault::ReadProt,
        fault
    );
}

bool VxpMemory::write(uint32_t address, const uint8_t* data, size_t len,
                      ArmFault* fault) {
    return accessChecked(
        address, nullptr, data, len,
        VXP_PROT_WRITE,
        ArmFault::WriteUnmapped,
        ArmFault::WriteProt,
        fault
    );
}

bool VxpMemory::fetch(uint32_t address, uint8_t* out, size_t len,
                      ArmFault* fault) {
    return accessChecked(
        address, out, nullptr, len,
        VXP_PROT_EXEC,
        ArmFault::FetchUnmapped,
        ArmFault::FetchProt,
        fault
    );
}
