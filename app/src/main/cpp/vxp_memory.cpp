...
namespace {
inline uint64_t pageIndexOf(uint64_t address) { return address / VxpMemory::kPageSize; }
inline uint64_t alignDown(uint64_t addr) { return addr & ~(VxpMemory::kPageSize - 1); }
} // namespace
...
bool VxpMemory::mapRegion(uint64_t base, uint64_t size, uint32_t perms,
    ...
    uint64_t pageCount = alignedSize / kPageSize;
...
bool VxpMemory::apiRead(uint64_t address, uint8_t* out, size_t len) const {
    for (size_t i = 0; i < len; i++) {
        ...
        out[i] = page ? page->data[(address + i) % kPageSize] : 0;
...
bool VxpMemory::apiWrite(uint64_t address, const uint8_t* data, size_t len) {
    for (size_t i = 0; i < len; i++) {
        ...
        page->data[addr % kPageSize] = data[i];
    }
    return true;
}
...
bool VxpMemory::accessChecked(uint32_t address, uint8_t* out, const uint8_t* in, size_t len,
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (out != nullptr) out[i] = page->data[addr % kPageSize];
        if (in != nullptr) page->data[addr % kPageSize] = in[i];
    }
    return true;
}
