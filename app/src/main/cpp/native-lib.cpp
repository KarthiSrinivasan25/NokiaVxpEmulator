...
Java_com_nokia_vxp_nativecore_NativeBridge_getNativeVersion(JNIEnv *env, jobject /* this */) {
    ...
    std::string version = "NokiaVxpEmulator native core - custom ARMv4T interpreter "
                           "(no external CPU emulation library), cpu/memory/mre bridges active";
...
Java_com_nokia_vxp_nativecore_NativeBridge_nativeInit(JNIEnv *env, jobject /* this */) {
    // Real per-session init (ARM interpreter engine creation, memory
    // mapping, dispatch hook installation) happens per-load in memory/MemoryManager,
