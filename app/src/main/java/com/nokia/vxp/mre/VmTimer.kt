 ...
 * emulator.TimerManager. Naming/signature (period in ms + repeat flag)
 * is a plausible placeholder, not confirmed against a real SDK header.
 */
object VmTimer {
    fun registerHandlers(dispatcher: VmDispatcher, timerManager: TimerManager) {
        ...
        dispatcher.registerHandler("vm_timer_create", VmApiTable.TIMER_CREATE) { args ->
        ...
        dispatcher.registerHandler("vm_timer_delete", VmApiTable.TIMER_DELETE) { args ->
            // r0 = timer id previously returned by vm_timer_create
